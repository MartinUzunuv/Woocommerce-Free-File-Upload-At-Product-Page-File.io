// Display a file upload field and hidden custom note field conditionally
add_action('woocommerce_before_add_to_cart_button', 'add_conditional_file_upload_field');
function add_conditional_file_upload_field() {
    echo '<div class="custom-video-option">';
    echo '<label for="video-fajl">Video File:</label>';
    echo '<select id="video-fajl" name="video_option">';
    echo '<option value="none">Without File</option>';
    echo '<option value="video">With file</option>';
    echo '</select>';
    echo '</div>';
...
add_filter('woocommerce_add_to_cart_validation', 'validate_custom_note_field', 10, 3);
function validate_custom_note_field($passed, $product_id, $quantity) {
...
add_filter('woocommerce_add_cart_item_data', 'save_custom_note_field', 10, 2);
function save_custom_note_field($cart_item_data, $product_id) {
...
add_action('woocommerce_checkout_create_order_line_item', 'save_custom_note_to_order', 10, 4);
function save_custom_note_to_order($item, $cart_item_key, $values, $order) {
